...
async def purge_cache() -> bool:
    ...
    if not ENABLE_GLOBAL_CDN or not CLOUD_CDN_API_TOKEN:
        ...
        return False
    api_url = os.getenv("CDN_PURGE_URL", "https://api.cloudflare.com/client/v4/zones/purge_cache")
    headers = {"Authorization": f"Bearer {CLOUD_CDN_API_TOKEN}"}
    try:
        async with httpx.AsyncClient() as client:
            resp = await client.post(api_url, headers=headers, json={"purge_everything": True}, timeout=10.0)
