...
PROMPT_ROUTER_SCHEME = os.getenv("PROMPT_ROUTER_SCHEME", "http")
LOCAL_LLM_API_URL = (
    f"{PROMPT_ROUTER_SCHEME}://{PROMPT_ROUTER_HOST}:{PROMPT_ROUTER_PORT}/route"
)
...
def is_path_disallowed(path):
    ...
    try:
        ...
        logger.debug("Failed to check disallowed path %s: %s", path, exc)
        return False
    return False
...
def extract_features(
    ...
    try:
        ...
        logger.debug("Failed to parse referer %s: %s", referer, exc)
    ...
    if timestamp_val:
        try:
            ...
            logger.debug("Failed to parse timestamp %s: %s", timestamp_val, exc)
