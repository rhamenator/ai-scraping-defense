...
CAPTCHA_VERIFICATION_URL = os.getenv("CAPTCHA_VERIFICATION_URL", "https://www.google.com/recaptcha/api/siteverify")
...
async def verify_captcha(token: str, ip: str):
    if not CAPTCHA_SECRET:
        ...
        raise HTTPException(status_code=500, detail="CAPTCHA verification not configured")
    payload = {"secret": CAPTCHA_SECRET, "response": token, "remoteip": ip}
    try:
        async with httpx.AsyncClient() as client:
            resp = await client.post(CAPTCHA_VERIFICATION_URL, data=payload, timeout=10.0)
