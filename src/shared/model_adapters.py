...
class MCPAdapter(BaseModelAdapter):
    """Adapter that proxies predictions to an MCP tool."""

    def _load_model(self):
        self.tool_name: Optional[str] = None
        self.client: Optional[MCPClient] = None
        config_map = self.config or {}
        try:
            server_label, tool_name, query = parse_mcp_uri(self.model_uri)
        except ValueError as exc:
            logging.error(f"Invalid MCP MODEL_URI '{self.model_uri}': {exc}")
            return

        overrides: Dict[str, Any] = dict(query)
    ...
    def predict(self, data: Any, **kwargs) -> Dict[str, Any]:
        ...
        extra = dict(kwargs)
        timeout_override = extra.pop("tool_timeout", None) or extra.pop("timeout", None)
    ...
    def _prepare_payload(self, data: Any, extra: Dict[str, Any]) -> Dict[str, Any]:
        payload: Dict[str, Any]
        if self.forward_full_payload and isinstance(data, dict):
            payload = dict(data)
        else:
            payload = {}
            key = self.argument_key
            if key:
                payload[key] = data
