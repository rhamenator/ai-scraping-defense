...
class WorkStealingQueue:
    ...
    def put(
        self, item: tuple[Callable, tuple, dict], timeout: Optional[float] = None
    ) -> bool:
...
class OptimizedThreadPoolExecutor:
    """Thread pool executor with dynamic sizing and monitoring."""
    ...
    def __init__(self, config: Optional[ThreadPoolConfig] = None):
        ...
        if self.config.enable_monitoring:
            self._monitor_thread = threading.Thread(
                target=self._monitor_loop, daemon=True, name="ThreadPoolMonitor"
            )
    ...
    def _load_config(self) -> ThreadPoolConfig:
        ...
        return ThreadPoolConfig(
            ...
            enable_monitoring=os.getenv("THREAD_POOL_MONITORING", "true").lower()
            == "true",
            scaling_factor=float(os.getenv("THREAD_POOL_SCALING_FACTOR", "1.5")),
            scale_down_threshold=float(
                os.getenv("THREAD_POOL_SCALE_DOWN_THRESHOLD", "0.3")
            ),
        )
    ...
    def _init_prometheus_metrics(self) -> None:
        ...
        self._prom_tasks_completed = Counter(
            "thread_pool_tasks_completed_total", "Total number of tasks completed"
        )
        self._prom_tasks_failed = Counter(
            "thread_pool_tasks_failed_total", "Total number of tasks that failed"
        )
        self._prom_active_workers = Gauge(
            "thread_pool_active_workers", "Current number of active worker threads"
        )
        self._prom_queue_size = Gauge(
            "thread_pool_queue_size", "Current size of the work queue"
        )
        self._prom_execution_time = Histogram(
            "thread_pool_task_execution_seconds", "Task execution time in seconds"
        )
    ...
    def _resize_pool(self, new_size: int) -> None:
        ...
        with self._lock:
            ...
            logger.info(
                f"Resizing thread pool from {current_size} to {new_size} workers"
            )

            # Create new executor with updated size
            old_executor = self._executor
            self._executor = ThreadPoolExecutor(
                max_workers=new_size, thread_name_prefix="OptimizedWorker"
            )
    ...
    def _should_scale_up(self) -> bool:
        ...
        queue_size = self._pending_tasks
        ...
        if len(self._execution_times) >= 10:
            ...
            if (
                recent_avg
                > self._metrics.avg_execution_time * self.config.scaling_factor
            ):
                return True
    ...
    def _monitor_loop(self) -> None:
        ...
        while not self._shutdown:
            try:
                ...
                # Update metrics
                if PROMETHEUS_AVAILABLE and self.config.enable_monitoring:
                    self._prom_queue_size.set(self._pending_tasks)
            ...
            try:
                ...
                with self._lock:
                    ...
                    if self._execution_times:
                        self._metrics.avg_execution_time = sum(
    ...
    def get_metrics(self) -> ThreadPoolMetrics:
        ...
        with self._lock:
            self._metrics.queue_size = self._pending_tasks
