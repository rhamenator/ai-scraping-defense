...
class WebhookEvent(BaseModel):
    ...
    timestamp_utc: Union[str, datetime.datetime] = Field(..., description="Timestamp of the original detection")
...
def add_ip_to_blocklist(ip_address: str, reason: str, event_details: Optional[dict] = None) -> bool:
    ...
    try:
        ...
        log_event(BLOCK_LOG_FILE, "BLOCKLIST_ADD_TTL", {
                  "ip_address": ip_address, "reason": reason, "ttl_seconds": BLOCKLIST_TTL_SECONDS, "details": event_details})
        return True
...
async def report_ip_to_community(ip: str, reason: str, details: dict) -> bool:
    if not ENABLE_COMMUNITY_REPORTING or not COMMUNITY_BLOCKLIST_REPORT_URL or not COMMUNITY_BLOCKLIST_API_KEY or not ip:
        if ENABLE_COMMUNITY_REPORTING and ip:
            logger.debug(f"Community reporting skipped for IP {ip}: URL or API Key not configured.")
        return False
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            log_event(COMMUNITY_REPORT_LOG_FILE, "COMMUNITY_REPORT_SUCCESS",
...
async def send_generic_webhook_alert(event_data: WebhookEvent):
    ...
    try:
        ...
        log_error(f"Failed to send generic webhook alert to {ALERT_GENERIC_WEBHOOK_URL} for IP {ip}", e)
    except httpx.HTTPStatusError as e:
        log_error(f"Generic webhook alert failed for IP {ip} with status {e.response.status_code}", e)
    except Exception as e:
    ...
    def smtp_send_sync():
        ...
        try:
            ...
            if ALERT_SMTP_PORT == 465:
                smtp_conn = smtplib.SMTP_SSL(str(ALERT_SMTP_HOST), ALERT_SMTP_PORT,
            ...
            if ALERT_SMTP_USE_TLS and ALERT_SMTP_PORT != 465:
                smtp_conn.starttls(context=ssl.create_default_context())  # Don't call starttls if already SSL
...
async def send_alert(event_data: WebhookEvent):
    severity_map = {"High Heuristic": 1, "Local LLM": 2, "External API": 3,
                    "High Combined": 1, "Honeypot_Hit": 2, "IP Reputation": 1}
    ...
    if event_severity < min_severity:
        logger.debug(f"Skipping alert for IP {event_data.details.get('ip')}. Severity {event_severity} < Min {min_severity}")
        return
    logger.info(
