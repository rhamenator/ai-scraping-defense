...
PG_PASSWORD_FILE = os.getenv(
    "TRAINING_PG_PASSWORD_FILE", "./secrets/training_pg_password.txt"
)
...
MIN_SAMPLES_FOR_TRAINING = int(
    os.getenv("MIN_SAMPLES_FOR_TRAINING", os.getenv("TRAINING_MIN_SAMPLES", 100))
)
...
KNOWN_BENIGN_CRAWLERS_UAS_STR = os.getenv(
...
def _get_pg_password(password_file_path: str) -> Optional[str]:
    ...
    paths_to_try = [
        ...
        os.path.join(
            ...
            "..",
            "secrets",
            os.path.basename(password_file_path),
        ),
    ]
    for path in paths_to_try:
        if os.path.exists(path):
            try:
                ...
                print(
                    "Warning: Failed to read PostgreSQL password from one of the configured paths."
                )
                continue
    print(
        f"Error: PostgreSQL password file not found at specified path or fallbacks: {password_file_path}"
    )
    return None
...
def load_and_process_logs(conn: psycopg2.extensions.connection) -> pd.DataFrame:
    ...
    df["time_since_last_sec"] = (
        df_sorted.groupby("ip")["timestamp"].diff().dt.total_seconds().fillna(-1)
    )
    df["req_freq_60s"] = df_sorted.rolling(
        "60s", on="timestamp", closed="left"
    ).count()["ip"]
    ...
    df["ua_is_known_bad"] = ua_lower.apply(
        lambda x: 1 if any(bad in x for bad in KNOWN_BAD_UAS) else 0
    )
    df["ua_is_known_benign_crawler"] = ua_lower.apply(
        lambda x: 1 if any(good in x for good in KNOWN_BENIGN_CRAWLERS_UAS) else 0
    )
    ...
    df["path_is_wp"] = (
        df["path"].str.contains(r"/wp-|/xmlrpc\.php", regex=True).astype(int)
    )
        ...
        with open(honeypot_path, "r") as f:
            ...
            for line in content.splitlines():
                try:
                    ...
                    print(f"Error processing honeypot log line: {e}")
                    continue
        ...
        with open(captcha_path, "r") as f:
            ...
            for line in content.splitlines():
                try:
                    ...
                    print(f"Error processing CAPTCHA log line: {e}")
                    continue
...
# --- Labeling ---
def assign_labels_and_scores(
    df: pd.DataFrame, honeypot_triggers: set, captcha_successes: set
) -> pd.DataFrame:
    ...
    if "ua_is_known_bad" not in df.columns:
        ...
        df["ua_is_empty"] = (df["ua_length"] == 0).astype(int)
        df["ua_is_known_bad"] = ua_lower.apply(
            lambda x: 1 if any(b in x for b in KNOWN_BAD_UAS) else 0
        )
        df["ua_is_known_benign_crawler"] = ua_lower.apply(
            lambda x: 1 if any(g in x for g in KNOWN_BENIGN_CRAWLERS_UAS) else 0
        )
    ...
    def calculate_score(row):
        ...
        if row["ua_is_known_bad"] and not row["ua_is_known_benign_crawler"]:
            score += 0.45
            reasons.append("KnownBadUA")
        if row["ua_is_empty"]:
            score += 0.30
            reasons.append("EmptyUA")
        if row["path_disallowed"] and not row["ua_is_known_benign_crawler"]:
            score += 0.35
            reasons.append("DisallowedPath")
        if row["status"] > 400:
            score += 0.10
            reasons.append(f"ClientError_{row['status']}")
        if row["req_freq_60s"] > 50:
            score += 0.15
            reasons.append(f"HighFreq_{int(row['req_freq_60s'])}")
        if row["time_since_last_sec"] != -1 and row["time_since_last_sec"] < 0.5:
            score += 0.10
            reasons.append("VeryFastRepeat")

        # Apply deductions
        if row["ua_is_known_benign_crawler"]:
            score -= 0.60
            reasons.append("KnownBenignUA")
        if not row["referer_is_empty"]:
            score -= 0.10
            reasons.append("HasReferer")

        return max(0.0, min(1.0, score)), reasons

    # Apply scoring to all rows at once
    df[["bot_score", "labeling_reasons"]] = df.apply(
        calculate_score, axis=1, result_type="expand"
    )
...
def train_and_save_model(df: pd.DataFrame, model_path: str, model_type: str = "rf"):
    ...
    if len(high_conf_df) < MIN_SAMPLES_FOR_TRAINING:
        print(
            f"Skipping RF training: Only {len(high_conf_df)} high-confidence samples, need {MIN_SAMPLES_FOR_TRAINING}."
        )
        return
    ...
    high_conf_df["is_bot"] = high_conf_df["label"].apply(
        lambda x: 1 if x == "bot" else 0
    )
...
def save_data_for_finetuning(
    df: pd.DataFrame, train_file: str, eval_file: str, eval_ratio: float
):
            ...
            with open(file_path, "w", encoding="utf-8") as f:
                for entry in data_to_write:
                    log_data_for_llm = {
                        ...
                        for k in entry
                        if k
                        not in ["label", "bot_score", "labeling_reasons", "timestamp"]
                    }
                    output_entry = {
...
if __name__ == "__main__":
    print(
        "--- Starting Bot Detection Model Training & Data Export (High-Performance Version) ---"
    )
    ...
    try:
        ...
        if not df.empty:
            ...
            labeled_df = assign_labels_and_scores(
                df, honeypot_triggers, captcha_successes
            )
