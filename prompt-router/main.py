...
async def _dispatch_prompt(target: str, payload: dict) -> dict:
    if target.startswith("mcp://"):
        try:
            return await asyncio.to_thread(call_mcp_tool, target, payload)
...
async def route_prompt(request: Request, response: Response) -> dict:
    ...
    try:
        return await _dispatch_prompt(target, payload)
