...
def group_problems(problems):
    grouped = defaultdict(lambda: {"affected_files": [], "description": "", "severity": "", "fix_prompt": ""})
    
    for p in problems:
        # Key by problem title/name
        title = p.get("problem") or p.get("message") # 'problem' in json, 'message' in txt
        if not title:
            continue
        ...
        if "resource" in p: # from problems.txt
            grouped[key]["affected_files"].append(f"{p['resource']}:{p.get('startLineNumber')}")
...
def check_if_issue_exists(title):
    ...
    try:
        ...
        cmd = ["gh", "issue", "list", "--search", f"{title} in:title", "--state", "all", "--json", "title"]
...
def upload_issues(grouped_problems):
    ...
    for title, data in grouped_problems.items():
        ...
        try:
            cmd = ["gh", "issue", "create", "--title", title, "--body", body, "--label", "automated-issue"]
...
def main():
    ...
    parser.add_argument("--report", action="store_true", default=True, help="Generate Markdown report (default)")
