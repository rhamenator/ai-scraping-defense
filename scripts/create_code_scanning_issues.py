...
def group_bandit_issues(results: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
...
def main():
    ...
    if not os.environ.get('GH_TOKEN'):
        ...
        print("Issue creation will fail. This is expected in the automated environment.")
    ...
    for test_id, issues in sorted(grouped_issues.items()):
        first_issue = issues[0]
        severity = first_issue.get('issue_severity', 'UNKNOWN')
        title = f"[Codacy] {test_id}: {first_issue.get('test_name', 'Unknown')} " \
