...
"""
    ...
    try:
        ...
        if response:
            ...
            return result["severity"], float(result["confidence"]), result.get("reasoning", "")
...
def main():
    ...
    problems = [(title, details) for title, details in data.items() if title not in processed]
    total = len(problems)
    ...
    for i, (title, details) in enumerate(problems):
        ...
        if not severity or not confidence:
            severity, confidence = heuristic_severity_confidence(category, proposed_count, file_count)
