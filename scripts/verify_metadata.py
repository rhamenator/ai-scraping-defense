...
print("\n=== CONFIDENCE STATS ===")
stats = c.execute('SELECT AVG(confidence), MIN(confidence), MAX(confidence) FROM problems').fetchone()
print(f"  Average: {stats[0]:.3f}")
print(f"  Min: {stats[1]}")
print(f"  Max: {stats[2]}")

print("\n=== SAMPLE PROBLEMS ===")
for row in c.execute('SELECT title, category, severity, ROUND(confidence, 2) FROM problems LIMIT 10'):
    print(f"  {row[0][:50]:50} | {row[1]:12} | {row[2]:6} | {row[3]}")

print("\n=== PR TRACKING FIELDS ===")
created_count = c.execute('SELECT COUNT(*) FROM problems WHERE pr_created = 1').fetchone()[0]
