...
# Prefer the system Docker Engine on servers (avoids accidental Docker Desktop context).
# Override as needed, e.g. DOCKER_CONTEXT=desktop-linux.
DOCKER_CONTEXT=${DOCKER_CONTEXT:-default}

# Base docker invocation with context.
docker_ctx() {
...
compose() {
  if command -v docker >/dev/null 2>&1 && $(docker_ctx) compose version >/dev/null 2>&1; then
    echo "$(docker_ctx) compose"
...
defense_network() {
  ...
  for svc in nginx_proxy apache_proxy admin_ui ai_service; do
    if $(docker_ctx) ps -a --format '{{.Names}}' | grep -qx "$svc"; then
      local nets
      nets=$(
        $(docker_ctx) inspect "$svc" -f \
  ...
  local name
  name=$(
    $(docker_ctx) network ls --format '{{.Name}}' \
      | grep -E '(^defense_network$|_defense_network$)' | head -n1 || true
  )
  if [ -n "${name:-}" ]; then
    echo "$name"
    return 0
  ...
  name=$(
    $(docker_ctx) network ls --format '{{.Name}}' \
      | grep 'defense_network' | head -n1 || true
  )
...
wait_for_container_healthy() {
  ...
  while :; do
    local status
    status=$(
      $(docker_ctx) inspect -f '{{.State.Health.Status}}' "$name" 2>/dev/null || true
    )
...
wait_for_mariadb() {
  ...
  start=$(date +%s)
  while :; do
    if $(docker_ctx) exec "$name" sh -lc "mysqladmin ping -h 127.0.0.1 -uroot -pexample --silent" >/dev/null 2>&1; then
      return 0
    fi
    local now
    now=$(date +%s)
