...
local exists, exists_err = red:exists(blocklist_ip_key)

if exists_err then
  ngx.log(
    ngx.ERR,
    "check_blocklist: Failed to query Redis (EXISTS ",
    blocklist_ip_key,
    ") for IP ",
    remote_addr,
    ": ",
...
-- Example debug log:
-- ngx.log(ngx.DEBUG, "check_blocklist: IP ", remote_addr, " not found in blocklist")
-- ngx.log(ngx.DEBUG, "check_blocklist: Key: '", blocklist_ip_key, "'. Allowing request.")
return
