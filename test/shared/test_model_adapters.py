...
class TestModelAdaptersComprehensive(unittest.TestCase):
    ...
    def test_sklearn_adapter_success(self):
        ...
        np.testing.assert_array_equal(
            predictions, mock_model.predict_proba.return_value
        )
    ...
    def test_markov_adapter_success(self):
        ...
        with patch(
            "src.shared.model_adapters.generate_dynamic_tarpit_page",
            ...
            with patch("src.shared.model_adapters.MARKOV_AVAILABLE", True):
                adapter = MarkovAdapter(
                    model_uri="N/A"
                )  # URI is not used for this adapter
    ...
    def test_http_adapter_success(self, mock_http_client):
        ...
        adapter = HttpModelAdapter(
            model_uri="http://model-api/predict", config={"api_key": "test-key"}
        )
        ...
        self.assertIn(
            "Authorization", mock_client_instance.post.call_args[1]["headers"]
        )
        self.assertEqual(
            mock_client_instance.post.call_args[1]["headers"]["Authorization"],
            "Bearer test-key",
        )
    ...
    def test_http_adapter_api_error(self, mock_http_client):
        ...
        mock_client_instance.post.return_value.raise_for_status.side_effect = (
