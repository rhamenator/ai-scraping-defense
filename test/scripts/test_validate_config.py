...
def test_check_drift_injects_cli_environment(tmp_path: Path) -> None:
    env_file = tmp_path / ".env"
    env_file.write_text("MODEL_URI=sklearn:///model\nTENANT_ID=tenant-123\n")

    loader = mock.Mock()
    loader.load_from_env.return_value = mock.Mock(to_dict=mock.Mock(return_value={}))
    drift_detector = mock.Mock()
    drift_detector.detect_drift.return_value = (False, [], {})

    with mock.patch.object(
        validate_config, "ConfigLoader", return_value=loader
    ), mock.patch.object(validate_config, "ConfigDrift", return_value=drift_detector):
        result = validate_config.check_drift(env_file, environment="staging")

    assert result is True
    passed_env = loader.load_from_env.call_args.args[0]
    assert passed_env["APP_ENV"] == "staging"
    drift_detector.detect_drift.assert_called_once()


def test_check_drift_fails_on_environment_conflict(tmp_path: Path) -> None:
    env_file = tmp_path / ".env"
    env_file.write_text(
        "MODEL_URI=sklearn:///model\nTENANT_ID=tenant-123\nAPP_ENV=production\n"
    )
