...
class TestIPFlaggerComprehensive(unittest.TestCase):
    ...
    def test_flag_suspicious_ip_success(self):
        ...
        self.mock_get_redis.assert_called_once_with(
            db_number=ip_flagger.REDIS_DB_FLAGGED_IPS
        )
        self.mock_redis_client.setex.assert_called_once_with(
            "default:ip_flag:8.8.8.8",
            ip_flagger.FLAGGED_IP_TTL_SECONDS,
            "Test Reason: High frequency",
        )
    ...
    def test_remove_ip_flag_success(self):
        ...
        self.mock_redis_client.delete.assert_any_call(
            f"{ip_flagger.FLAG_COUNT_PREFIX}8.8.8.8"
        )
