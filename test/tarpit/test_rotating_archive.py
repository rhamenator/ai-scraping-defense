...
class TestRotatingArchiveComprehensive(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patches = {
            "ARCHIVE_DIR": patch(
                "src.tarpit.rotating_archive.ARCHIVE_DIR", self.test_dir
            ),
            "ARCHIVE_PATTERN": patch(
                "src.tarpit.rotating_archive.ARCHIVE_PATTERN",
                os.path.join(self.test_dir, "assets_*.zip"),
            ),
            "MAX_ARCHIVES_TO_KEEP": patch(
                "src.tarpit.rotating_archive.MAX_ARCHIVES_TO_KEEP", 3
            ),
            "create_fake_js_zip": patch(
                "src.tarpit.rotating_archive.create_fake_js_zip"
            ),
        }
        self.mocks = {name: patcher.start() for name, patcher in self.patches.items()}

        # Configure the mock for create_fake_js_zip
        self.new_archive_path = os.path.join(
            self.test_dir, f"assets_{int(time.time())}.zip"
        )
    ...
    def test_rotate_archives_deletes_oldest_files(self):
        ...
        self.mocks["create_fake_js_zip"].assert_called_once_with(
    ...
    def test_rotate_archives_not_enough_files(self):
        ...
        )  # os.remove is mocked so nothing is deleted from our setup
    ...
    def test_rotate_archives_handles_creation_failure(self):
        ...
        self.mocks["create_fake_js_zip"].return_value = (
    ...
    def test_rotate_archives_no_existing_files(self, mock_glob):
        ...
        )  # Nothing actually gets created because of the mock

    @patch("os.remove", side_effect=OSError("Permission denied"))
    @patch("src.tarpit.rotating_archive.logger.error")
    def test_rotate_archives_handles_deletion_error(
        self, mock_logger_error, mock_os_remove
    ):
        ...
        self.assertIn(
            "Failed to delete old archive", mock_logger_error.call_args_list[0][0][0]
        )
