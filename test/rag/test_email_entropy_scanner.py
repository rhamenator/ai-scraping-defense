...
class TestEmailEntropyScannerFunctions(unittest.TestCase):
    ...
    def test_calculate_entropy(self):
        ...
        self.assertEqual(
            calculate_entropy(""), 0.0, "Entropy of empty string should be 0."
        )
        self.assertEqual(
            calculate_entropy("aaaaa"),
            0.0,
            "Entropy of a single repeated character should be 0.",
        )
        self.assertAlmostEqual(
            calculate_entropy("ab"), 1.0, msg="Entropy of 'ab' should be 1.0."
        )
        self.assertAlmostEqual(
            calculate_entropy("abc"),
            math.log2(3),
            places=5,
            msg="Entropy of 'abc' should be log2(3).",
        )
        entropy_test = calculate_entropy("hello")
        expected_entropy_hello = -(
            3 * (1 / 5) * math.log2(1 / 5) + (2 / 5) * math.log2(2 / 5)
        )
        self.assertAlmostEqual(entropy_test, expected_entropy_hello, places=5)
        self.assertGreater(
            calculate_entropy("abcdefghijklmnopqrstuvwxyz"),
            4.0,
            "Entropy of diverse string should be high.",
        )
        self.assertLess(
            calculate_entropy("aaabbbccc"),
            calculate_entropy("abcdefghi"),
            "More repetition means lower entropy.",
        )

    def test_is_suspicious_username(self):
        """Test username suspicion logic."""
        self.assertTrue(
            is_suspicious_username(""), "Empty username should be suspicious."
        )
        self.assertTrue(
            is_suspicious_username("xg8u9h13g51ab"), "High entropy, length > 12"
        )  # len 13
        self.assertTrue(
            is_suspicious_username("123456789012345"), "High digit ratio, length > 12"
        )  # len 15
        self.assertTrue(
            is_suspicious_username("sdfghjklmnbvcxzqw"),
            "No vowels, high entropy, length > 12",
        )  # len 17

        self.assertFalse(
            is_suspicious_username("jane.doe"),
            "Normal username should not be suspicious.",
        )
        self.assertFalse(is_suspicious_username("support"), "Normal username.")
        self.assertFalse(
            is_suspicious_username("a.very.long.username.with.dots"),
            "Long but low entropy username.",
        )
        ...
        self.assertFalse(
            is_suspicious_username("x1zqrs"),
            "Short, higher entropy but has vowel and not excessively long.",
        )
        self.assertFalse(
            is_suspicious_username("contact123"), "Contains digits but overall normal."
        )
        self.assertFalse(
            is_suspicious_username("abcdefghijk"),
            "len=11, entropy high, but has vowels, no digits",
        )
        self.assertTrue(
            is_suspicious_username("qwrtypsdfghjkl"),
            "len=14, high entropy, no vowels -> True",
        )
    ...
    def test_is_disposable_domain(self):
        ...
        self.assertTrue(
            is_disposable_domain("TEMPMAIL.COM", disposable_list),
        ...
        self.assertTrue(
            is_disposable_domain("", disposable_list),
            "Empty domain should be treated as suspicious (or disposable in this context).",
        )
    ...
    def test_is_suspicious_email(self):
        ...
        self.assertFalse(
            is_suspicious_email("john.doe@gmail.com", disposable_list), "Normal email."
        )
        self.assertTrue(
            is_suspicious_email("randomstring@mailinator.com", disposable_list),
            "Disposable domain.",
        )
        self.assertTrue(
            is_suspicious_email("qwerty123zxcvbnm@example.com", disposable_list),
            "Suspicious username part.",
        )
        self.assertTrue(
            is_suspicious_email("qwrtypsdfghjkl@example.com", disposable_list),
            "Suspicious username (no vowels, high entropy).",
        )

        self.assertTrue(
            is_suspicious_email("plainaddress", disposable_list),
            "Invalid format (no @).",
        )
        self.assertTrue(is_suspicious_email("", disposable_list), "Empty email string.")
        self.assertTrue(
            is_suspicious_email("@domain.com", disposable_list),
            "Missing username part.",
        )
        self.assertTrue(
            is_suspicious_email("user@", disposable_list), "Missing domain part."
        )
        self.assertTrue(
            is_suspicious_email("user@.com", disposable_list),
            "Effectively empty domain part after @.",
        )
    ...
    def test_main_block_runs_examples(self, mock_print):
        ...
        with patch.object(email_entropy_scanner, "__name__", "__main__"):
            ...
            test_emails_in_main = ["jane.doe@gmail.com", "b394v8n93n4v@tempmail.com"]
            ...
            for email_in_main in test_emails_in_main:
                result = is_suspicious_email(
                    email_in_main, default_disposable_domains_in_main
                )
