...
class TestFinetuneScriptComprehensive(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open(self.train_file, "w") as f:
            f.write(
                json.dumps({"log_data": json.dumps(self.log_entry_bot), "label": "bot"})
                + "\\n"
            )
        with open(self.eval_file, "w") as f:
            f.write(
                json.dumps(
                    {"log_data": json.dumps(self.log_entry_human), "label": "human"}
                )
                + "\\n"
            )
    ...
    def test_load_and_prepare_dataset(self, mock_load_dataset):
        ...
        prepared_data = finetune.load_and_prepare_dataset(
            self.train_file, mock_tokenizer
        )
