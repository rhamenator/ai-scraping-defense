...
class TestTrainingPipelineComprehensive(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open(self.pg_password_path, "w") as f: f.write("testpass")
    ...
    def test_train_and_save_model(self, mock_fit, mock_dump):
        ...
        df = pd.DataFrame({
            'label': ['bot']*5 + ['human']*5,
            'ua_length': range(10), 'status': [200]*10, 'bytes': range(100, 110),
            'path_depth': range(10), 'path_length': range(10, 20), 'path_is_root': [1,0]*5,
