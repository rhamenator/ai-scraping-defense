...
class TestEscalationEngineComprehensive(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.patchers = {
            ...
            "get_redis_connection": patch(
                "escalation.escalation_engine.get_redis_connection"
            ),
            # Correctly patch the module-level variable, not the function that creates it
            "model_adapter": patch(
                "escalation.escalation_engine.model_adapter", MagicMock()
            ),
            "check_ip_reputation": patch(
                "escalation.escalation_engine.check_ip_reputation",
                new_callable=AsyncMock,
            ),
            "get_frequency_features": patch(
                "escalation.escalation_engine.get_realtime_frequency_features"
            ),
            "classify_with_local_llm_api": patch(
                "escalation.escalation_engine.classify_with_local_llm_api",
                new_callable=AsyncMock,
            ),
            "classify_with_external_api": patch(
                "escalation.escalation_engine.classify_with_external_api",
                new_callable=AsyncMock,
            ),
            "forward_to_webhook": patch(
                "escalation.escalation_engine.forward_to_webhook",
                new_callable=AsyncMock,
            ),
            "trigger_captcha_challenge": patch(
        ...
        self.mocks["get_frequency_features"].return_value = {
            "count": 1,
            "time_since": 100,
        }
    ...
    def test_get_country_code_logs_lookup_failures(self):
        ...
        with (
            ...
            self.assertLogs(
                "src.escalation.escalation_engine", level="WARNING"
            ) as logs,
        ):
    ...
    async def test_escalate_invalid_ip_address(self):
        ...
        response = self.client.post(
            "/escalate", json=payload, headers={"X-API-Key": "testkey"}
        )
    ...
    async def test_escalate_missing_or_bad_api_key(self):
        ...
        with patch("src.escalation.escalation_engine.ESCALATION_API_KEY", "testkey"):
            ...
            resp_wrong = self.client.post(
                "/escalate", json=payload, headers={"X-API-Key": "wrong"}
            )
    ...
    async def test_classify_with_external_api_timeout_and_json_error(self):
        ...
        meta = escalation_engine.RequestMetadata(
            timestamp="2023-01-01T00:00:00Z", ip="1.1.1.1", source="test", method="GET"
        )
    ...
    async def test_escalate_endpoint_ip_reputation_block(self):
        ...
        self.mocks["check_ip_reputation"].return_value = {
            "is_malicious": True,
            "score": 99,
        }
        with (
