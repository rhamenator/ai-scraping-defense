...
| `SECURITY_CDN_TRUSTED_PROXY_CIDRS` / `SECURITY_CDN_ORIGIN_LOCKDOWN` | Trust Cloudflare proxy ranges for client IP restoration and optionally allowlist them at the nginx origin |
| `CLOUDFLARE_TUNNEL_TOKEN` | Optional token for `scripts/linux/start_cloudflare_tunnel.sh` named tunnel mode |
