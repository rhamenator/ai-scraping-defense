...
| `milestone` | string | Title of the milestone (or sub-milestone) selected for the issue. Sub-milestones use the format `Parent :: Severity`. | `"Local Stability :: High"` |
| `milestone_parent` | string or null | Parent milestone when a sub-milestone is used. Stores the base milestone that groups related work. | `"Local Stability"` |
| `milestone_number` | integer or null | GitHub milestone number backing the assignment. Populated when the milestone exists or is created through automation. | `42` |
| `milestone_url` | string or null | HTML link to the GitHub milestone so dashboards can provide quick navigation. | `"https://github.com/rhamenator/ai-scraping-defense/milestone/42"` |
...
- Milestone metadata is generated by `scripts/update_problem_milestones.py`, which synchronises JSON/SQLite state with live GitHub milestones and creates sub-milestones when helpful.
